...
const employeeAdd = async (req, res) => {
  try {
    const { name, email, age, description, departmentOption, reportTo } =
      req.body;
    if (!req.file) {
      return res.status(400).send("No file uploaded");
    }
    const image = req.file.filename;
    const Employeeadd = await employeesModel.create({
      name,
      email,
      age,
      description,
...
const getAllEmployees = async (req, res) => {
  try {
    ...
    const EmployeWithImages = employeesGet.map((employ) => {
      const imagePath = employ.image
        ? `http://localhost:5001/images/${employ.image}`
        : null;
...
const getEmployee = async (req, res) => {
  try {
    ...
    const imagePath = UpdateEmployee.image
      ? `http://localhost:5001/images/${UpdateEmployee.image}`
      : null;
    const EmployeDataWithImage = {
      ...UpdateEmployee.toObject(),
      image: imagePath,
    };
